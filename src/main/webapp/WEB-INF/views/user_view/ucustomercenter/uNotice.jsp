		...
		var pagingFunc = function(){
			...
			var renderTable = function(page){
				...
				for(var index = startNum; index < endNum; index++){
					var cat;
					if(list[index].noticeCategory == '이벤트'){
						cat = '<span style=\"color: #1f76bb;\">[이벤트]&nbsp;&nbsp;</span>';
					}else if(list[index].noticeCategory == '회원용-공지사항'){
						cat = '<span style=\"color: red;\">[공지사항]&nbsp;&nbsp;</span>';						
					}
			  		html += '<div class=\"minwoo_notice_one\">' + '<a href=\"/bomulsum/user/board/noticeDetail.do?seq='+ list[index].noticeSeq + '\">'
						+ cat + '<span>❑&nbsp;</span>' + '<span>' + list[index].noticeTitle + '</span></a>'
