...
<body id="page-top">
   ...
   <script>
      ...
      var goToMessage = function(e){
    	  e.preventDefault();
    	  alert($(this).attr('id'));
    	  window.location.href = '/bomulsum/writer/message.wdo?member='+$(this).attr('id');
      }
      ...
      var renderTable = function(){
         ...
         var artOptionAmount = [];
         var artPrice = [];
         var buyArtOptionPrice = [];
         var finalBuyTotalPrice = 0;
         ...
         for(var i=0; i<result.length; i++){
            ...
            //제일 상단
              ...
              labelCancelLoops:do {// result의 다음번 orderCodeSeq가 같을 경우, 동일한 구매 내역이므로 배송정보를 동일하게 입력할 수 있도록 반복문을 돌린다.
               ...
               buyArtOptionPrice = result[i].buyArtOptionPrice.split('|-%-|');
               finalBuyTotalPrice = 0;

               for(var p in buyArtOptionPrice){
                  finalBuyTotalPrice += parseInt(buyArtOptionPrice[p]);
               }
               ...
               html += '</div>'
                  ...
                  + '<div style="margin-top: 1rem">'
                  // 수정 부분
                  + '<a id="'+ result[i].memberCodeSeq +'" class="send_message_button" '
                  + 'style="text-decoration:none; border: 1px solid gray; color: #28E7FF; padding: 1%; font-weight: bold" href="">'
                  // 수정 수정
         ...
         $('.send_message_button').click(goToMessage);
