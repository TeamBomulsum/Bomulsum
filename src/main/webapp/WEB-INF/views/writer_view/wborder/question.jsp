<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="com.web.bomulsum.writer.board.repository.writerBoardDAOImpl" %>
<%@ page import="com.web.bomulsum.writer.board.repository.writerBoardVO" %>
...
<script type="text/javascript">
   $(function(){
      ...
      $('.writer_question_detail').click(function() {
         const param = $.trim($(this).closest('tr').children('td').eq(2).text());
         console.log('param : ', param);

         
         $.ajax({
            url: "${pageContext.request.contextPath}"+"/writer/questionArticle.wdo", //클라이언트가 HTTP 요청을 보낼 서버의 URL 주소
              ...
              <div class="table-responsive" style="scroll:no; overflow:hidden;">
                <table class="table table-bordered" id="dataTable" >
                  ...
                  <c:forEach var="writerBoardVO" items="${article}">
                     <tr>
                      <td data-toggle="modal" data-target="#staticBackdrop2" class="writer_question_detail">${writerBoardVO.admin_notice_title}</td>
                      <td>${writerBoardVO.admin_notice_date }</td>
                      <td style="display:none" class="admin_notice_seq">${writerBoardVO.admin_notice_seq}</td>
                    </tr>
                    </c:forEach>
                  </tbody>
                </table>
